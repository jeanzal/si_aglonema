...
class Obat extends CI_Controller
{
    ...
    public function list_data_obat()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('obat/data_obat/list', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function add_obat()
    {
        ...
        $data['title_dalam'] = "Tambah Data Obat";
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['data_jenis_obat'] = $this->Obat_M->get_data_jenis_obat();
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('obat/data_obat/add', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function edit_data_obat($id)
    {
        ...
        $data['title_dalam'] = "Update Jenis Obat";
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['edit_obat'] = $this->Obat_M->vEditDataObat($where, 'data_obat');
        $data['data_jenis_obat'] = $this->Obat_M->get_data_jenis_obat();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('obat/data_obat/edit', $data);
        $this->load->view('templates/footer');
    }
    public function update_obat()
    {
        ...
        if ($this->Obat_M->update_data_obat($where, $data, 'data_obat') == false) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-success" role="alert"> Data Berhasil diupdate </div>');
            redirect('obat/list_data_obat');
        } else {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger" role="alert"> Data Tidak Berhasil diupdate </div>');
